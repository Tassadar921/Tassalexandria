...
<ul class={`${footer ? '' : 'space-y-4'} p-4`}>
    {#if $profile}
        ...
        <MenuItem {footer} iconLeft="search" href="/browse" on:click>{$t('menu.browse')}</MenuItem>
        <MenuItem {footer} iconLeft="upload" href="/upload" on:click>{$t('menu.upload')}</MenuItem>
